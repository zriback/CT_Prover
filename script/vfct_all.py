...
def phasar(tkfile, taintentry, taintconfig, outfile, workdir = os.getcwd(), source_name=None):
...
def transfer(record,file,file2, recordfile = subprocess.PIPE, workdir = os.getcwd()):
    ...
    args = f"ruby -I {BAM991_LIB} {BAM991_BIN} --process_mark " + file + " -o " + file2
...
def verify_without_timelimit(file, resfile, workdir, error_limit):
    args = "boogie /doModSetAnalysis /errorLimit " + str(error_limit) + " " + file
    restime = runcommand(args, resfile, workdir=workdir)
    return restime
...
def one_and_two_and_three():
    ...
    for item in entry:
        ...
        restime5 = verify_without_timelimit(boolboogiefile, analyres, dirname, 5)

        t6 = markbpl(analyres, boolboogiefile, dirname)
        t7 = verify_without_timelimit(boolboogiefilenoloop, analyres2, dirname, 0)
        ...
        l2.append(restime3+restime4+restime5)
        l3.append(t6+t7)
        l4.append(t8+t9+t10+t11)
        total.append(restime1+ restime2 + restime3 + restime4 + restime5 + t6 + t7 + t8 + t9 + t10 + t11)

        i = i + 1

    detailtime_1_2_3.append(("1", l1))
    detailtime_1_2_3.append(("2", l2))
    detailtime_1_2_3.append(("2-2", l3))
    detailtime_1_2_3.append(("3", l4))
