...
if ! command -v ruby >/dev/null 2>&1; then
  echo "Error: ruby is not in PATH. Install Ruby (>= 2.7) before running this script." >&2
  exit 1
fi

if ! command -v gem >/dev/null 2>&1; then
  echo "Error: gem command not found. Ensure RubyGems is installed." >&2
  exit 1
fi

if ! command -v rake >/dev/null 2>&1; then
  echo "Warning: rake not found. Some BAM test suites require rake." >&2
fi
